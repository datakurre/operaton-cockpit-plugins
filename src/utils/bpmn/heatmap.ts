 ...
 */
export function elapsedOf(activity: HistoricActivityInstance, now: number = Date.now()): number {
  ...
  const running = now - Date.parse(activity.startTime);
 ...
 */
export function aggregateDurations(activities: HistoricActivityInstance[], now: number = Date.now()): HeatmapCell[] {
  ...
  for (const activity of activities) {
    ...
    totals.set(elementId, (totals.get(elementId) ?? 0) + elapsedOf(activity, now));
  }
