  ...
  if (favourites.length === 0) {
    return (
      <div style={{ padding: '20px' }}>
        ...
        <p>No favourite process definitions yet. Use the star button on a process definition to add it to favourites.</p>
      </div>
    );
  }
      ...
      <table className="table table-striped table-hover">
        ...
        <tbody>
          {favourites.map((fav) => (
            <tr key={fav.id}>
              <td>
                <a href={`#/process-definition/${fav.id}/runtime`}>
                  {fav.name || fav.key}
                </a>
              </td>
              <td>{fav.key}</td>
...
export default [
  ...
  {
    id: 'definitionFavouritesCockpitDashboard',
